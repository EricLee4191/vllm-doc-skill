...
- 2026-09-25：增量更新，基线 `9f07d023d0`（2026-09-23，v0.30.1rc0）→ `afea5c20c7`（2026-09-25，最新 tag 仍 **v0.30.1rc0**，`153242a314`），区间 127 commits（721 文件，+20634/−5948）。内容增量：`vllm preload` CLI（#56680，08 篇 §1.1）、slow tokenizer mode 移除（#58545）、`/v1/messages` Disable Thinking（#58613）、**DSpark PP 支持回退**（#56956 被 #58484 revert，03 篇 §6.2/05 篇 §2.2 同步修正）、DFlash context K/V precompute 进 draft CUDA graph（#57632）、Mamba2 prefill SSM state save 批量化（#49371）、xgrammar 原生 Lark grammar（#58321，07 篇 §3）、outlines EOS/mask 修复（#57743/#58612）、**PCP+DP 组合支持**（#57075，05 篇 §2.2）、SM100/103 low-SM multimem reduce-scatter（#55072）、XPU GRAPH 默认开（#51600）、ROCm gfx950 MXFP8 GEMM（#58510/#58456）、Triton kernel dispatcher（#43048）、FlashInfer 0.7.0（#58069）、fp8.py 在线量化移除（#53585）、DSv4 inverse RoPE+FP8 融合（#58621）、Engram huge pages（#56926）、KV connector finalize 修复（#57775）、多模态 block hashing/partial-block KV event 修复（#51694/#58288）、Rust 前端 lock-free histogram + SSE keep-alive（#58574/#58306）。锚点重锚：07 篇 xgrammar 6 处（backend_xgrammar.py 37→36/157→159/181→183/247→249/257→259）、03/05 篇 DSpark PP 段落按 revert 修正；零 diff 文件锚点保持原值。INDEX.md 已重新生成。
