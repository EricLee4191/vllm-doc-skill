...
> 知识库基于 vLLM `main`（`afea5c20c7`，2026-09-25）/ 最新 tag **v0.30.1rc0**（`153242a314`，2026-09-23，release candidate，HEAD 领先 147 commits；上一正式 release 为 v0.30.0，`9ed533eb4a`，2026-09-20）源码整理。vLLM 迭代很快，涉及"当前版本是否如此"的问题以源码为准。
...
│   └── INDEX.md                 #   tags 倒排索引（自动生成，642 tag）
└── scripts/
...
`knowledge/INDEX.md` 是这些 tags 的倒排索引（642 个 tag → `文件:行号`），可直接 grep 定位。若文档改动后索引过期，重建：
